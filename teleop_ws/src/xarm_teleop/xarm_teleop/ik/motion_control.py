...
class PinocchioMotionControl:
    def __init__(
        ...
        if arm_indices:
            ...
            locked_joint_ids = [i + 1 for i in locked_joint_ids]
        ...
        self.wrist_pose: pin.SE3 = pin.updateFramePlacement(self.model, self.data, self.wrist_id)
    ...
    def control(self, target_pos: np.ndarray, target_rot: np.ndarray) -> np.ndarray:
        ...
        qpos = self.qpos.copy()
        qpos = self.ik_clik(qpos, oMdes, self.wrist_id)
        self.qpos = pin.interpolate(self.model, self.qpos, qpos, self.alpha)
        self.qpos = qpos.copy()
        return self.qpos.copy()

    def ik_clik(self, qpos: np.ndarray, oMdes: pin.SE3, wrist_id: int, iter: int = 1000) -> np.ndarray:
        for _ in range(iter):
